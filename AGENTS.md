...
- 正確なライフログ要約へ寄せる
- クラウド推論を追加する
- 個人データを開発者サーバー、分析・広告基盤、用途不明な第三者へ送る
- 「成長して賢くなる」育成ゲームへ寄せる

ネットワークアクセスそのものは全面禁止ではない。
iOS標準のbackup / restoreやApple platform serviceを使うことだけを理由に却下しない。
...
- 失敗をskip/retryで隠さない。
- テスト未実施を成功と書かない。
- app-controlledな新規ネットワーク経路、とくにユーザー由来データを送る経路は `SECURITY.md` のreview対象にする。
- Apple frameworkの通常利用やOS-managed backupを、通信があるという理由だけで禁止しない。
